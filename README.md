...
[![version](https://img.shields.io/badge/version-1.1.1-blue?style=flat-square)](https://github.com/gajaguar/pylint-plugin)
...
**Prerequisite**: install Python 3.14 or higher. The package declares
`requires-python = ">=3.14"`.

Install [uv](https://docs.astral.sh/uv/) for dependency management and
virtual environments. Install [pnpm](https://pnpm.io/) (or another
Node package manager) only if you plan to run the Markdown lint and
spell targets.
...
Then sync and run pylint with the plugin loaded:
...
Clone the repository and install everything (Python deps, Node
toolchain, git hook):
...
- [x] 14 enforced checkers plus `app-smoke` registration probe
- [x] Configurable AAA section markers (option + env var)
- [x] Test-scoped activation for the `app-test-*` rules
...
Maintainer: <dev@gajaguar.com>

Repository:
[github.com/gajaguar/pylint-plugin](https://github.com/gajaguar/pylint-plugin)
